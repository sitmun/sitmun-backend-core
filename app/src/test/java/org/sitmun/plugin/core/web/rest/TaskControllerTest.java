...
import static org.sitmun.plugin.core.test.URIConstants.TASK_URI;
...
public class TaskControllerTest {
  ...
  public void saveValue() throws Exception {
    saved31507 = mockMvc.perform(get(TASK_URI, 31507)
  ...
  public void restoreValue() throws Exception {
    if (saved31507 != null) {
      mockMvc.perform(put(TASK_URI, 31507).content(saved31507)
          .with(SecurityMockMvcRequestPostProcessors.user(SITMUN_ADMIN_USERNAME)))
        .andExpect(status().isOk());
    }
  }
  ...
  public void ensureLegacyQueryTaskIsHandled() throws Exception {
    ...
    mockMvc.perform(put(TASK_URI, 31507).content(mustPassUpdate)
        .with(SecurityMockMvcRequestPostProcessors.user(SITMUN_ADMIN_USERNAME)))
      .andDo(print())
      .andExpect(status().isOk());
