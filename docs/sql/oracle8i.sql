...
CREATE TABLE STM_APP (
  ...
  APP_REFRESH   NUMBER(1) CHECK (APP_REFRESH IN (0, 1)),
  APP_ENTRYS    NUMBER(1) CHECK (APP_ENTRYS IN (0, 1)),
  APP_ENTRYM    NUMBER(1) CHECK (APP_ENTRYM IN (0, 1)),
  APP_GGIID     NUMBER(11),
  APP_CREATED   DATE,  -- Utilisation de DATE au lieu de TIMESTAMP
  APP_LOGO      VARCHAR2(4000),
  APP_MAINTENANCE_INFORMATION VARCHAR2(80),
  APP_UNAVAILABLE CHAR(1) DEFAULT '0' NOT NULL CHECK (APP_UNAVAILABLE IN ('0', '1')),
  APP_LAST_UPDATE DATE, -- Utilisation de DATE pour la compatibilité
  APP_CREATOR_ID NUMBER(11),
  APP_DESCRIPTION VARCHAR2(250),
  APP_PRIVATE   CHAR(1) DEFAULT '0' NOT NULL CHECK (APP_PRIVATE IN ('0', '1')),
