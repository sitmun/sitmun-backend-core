...
public class TaskEditCartographyService implements TaskMapper {
  ...
  @Autowired
  private DatabaseConnectionService dbConService;
    ...
    public TaskDto map(Task task, Application application, Territory territory) {
        ...
        boolean postRequest = false; //parameters.entrySet().stream().anyMatch(e -> "body".equals(((Map<String, Object>)e.getValue()).get("type")));
        ...
        parameters.put(AuthorizationConstants.TaskDto.PARAMETER_SERVICE, getParametersObject(paramType, true, service.getType()));
        String layers = cartography.getLayers().stream().reduce((a, b) -> a + "," + b).orElse("");
        if (DomainConstants.Services.isWfsService(service)) {
          parameters.put(AuthorizationConstants.TaskDto.PARAMETER_WFS_TYPENAME, getParametersObject(paramType, true, layers));
        } else {
          parameters.put(AuthorizationConstants.TaskDto.PARAMETER_LAYERS, getParametersObject(paramType, true, layers));
        }
     ...
     */
    private Map<String, Object> convertFieldsToJsonObject(Map<String, Object> properties, DatabaseConnection connection) {
        Map<String, Object> fields = new HashMap<>();
        
        @SuppressWarnings("unchecked")
        List<Map<String, Object>> listOfFields = (List<Map<String, Object>>) properties.getOrDefault("fields", Collections.emptyList());
        
        for (Map<String, Object> field : listOfFields) {
          if (field.containsKey("name") && field.containsKey("type") && field.containsKey("required") &&
            field.containsKey("selectable") && field.containsKey("editable") && field.containsKey("label")) {
            ...
            Map<String, Object> values = getFieldObject(label, type, required, selectable, editable, value, listValues);
     ...
     */
    private Map<String, Object> getFieldObject(String label, String type, Boolean required,
          Boolean selectable, Boolean editable, String value, List<Map<String, Object>> listValues) {
          Map<String, Object> values = new HashMap<>();
