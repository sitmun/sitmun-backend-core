...
final class PdfExternalResourceLoader {
  ...
  final class Session {
    ...
    private final long deadline =
        System.nanoTime() + TimeUnit.SECONDS.toNanos(MAX_DURATION_SECONDS);
    private int totalBytes;

    byte[] fetch(String uri) {
      byte[] cached = cache.get(uri);
      if (cached != null) return cached;
      if (!isAllowed(uri) || cache.size() >= MAX_RESOURCES || remainingTimeoutMillis() <= 0)
        return EMPTY;
      ...
      try {
        ...
        for (int redirects = 0; redirects <= MAX_REDIRECTS; redirects++) {
          ...
          OkHttpClient client =
              httpClient
                  .newBuilder()
                  .dns(requested -> pinned(requested, host, addresses))
          ...
          try (Response response =
              client
                  .newCall(new Request.Builder().url(current.toString()).get().build())
                  .execute()) {
    ...
    private byte[] decodeData(String uri) {
      ...
      try {
        return base64 ? Base64.getDecoder().decode(encoded) : decodePercent(encoded);
      } catch (IllegalArgumentException e) {
        return EMPTY;
      }
    }
      ...
      for (int i = 0; i < encoded.length(); ) {
        ...
        if (current == '%') {
          ...
          int high = Character.digit(encoded.charAt(i + 1), 16),
              low = Character.digit(encoded.charAt(i + 2), 16);
    ...
    private List<InetAddress> publicAddresses(URI uri) throws UnknownHostException {
      ...
      if (!("http".equalsIgnoreCase(scheme) || "https".equalsIgnoreCase(scheme))
          || host == null
          || uri.getUserInfo() != null) return List.of();
      host = normalizeHost(host);
      boolean ipv6 = host.contains(":");
      List<InetAddress> addresses = List.copyOf(dns.lookup(host));
      return !addresses.isEmpty()
              && !(ipv6 && addresses.stream().anyMatch(Inet4Address.class::isInstance))
              && addresses.stream().allMatch(PdfExternalResourceLoader::isPubliclyRoutable)
          ? addresses
          : List.of();
    }

    private List<InetAddress> pinned(
        String requested, String validated, List<InetAddress> addresses)
        throws UnknownHostException {
      if (!requested.equalsIgnoreCase(validated))
        throw new UnknownHostException("Unvalidated host: " + requested);
      return addresses;
    }
    ...
    private boolean isRedirect(int status) {
      return Set.of(301, 302, 303, 307, 308).contains(status);
    }
  ...
  static boolean isPubliclyRoutable(InetAddress address) {
    ...
    if (address instanceof Inet4Address)
      return !prefix(b, 0x00, 8)
          && !prefix(b, 0x0a, 8)
          && !prefix(b, 0x6440, 10)
          && !prefix(b, 0x7f, 8)
          && !prefix(b, 0xa9fe, 16)
          && !prefix(b, 0xac10, 12)
          && !prefix(b, 0xc00000, 24)
          && !prefix(b, 0xc00002, 24)
          && !prefix(b, 0xc05863, 24)
          && !prefix(b, 0xc0a8, 16)
          && !prefix(b, 0xc612, 15)
          && !prefix(b, 0xc63364, 24)
          && !prefix(b, 0xcb0071, 24)
          && !prefix(b, 0xe0, 4)
          && !prefix(b, 0xf0, 4);
    if (b.length != 16 || mapped(b) || starts(b, new byte[] {0, 100, (byte) 0xff, (byte) 0x9b}))
      return false;
    return !prefix(b, 0, 8)
        && !prefix(b, 0x0100000000000000L, 64)
        && !prefix(b, 0x200100, 23)
        && !prefix(b, 0x20010db8, 32)
        && !prefix(b, 0x2002, 16)
        && !prefix(b, 0x3ffe, 16)
        && !prefix(b, 0xfc, 7)
        && !prefix(b, 0xfec0, 10)
  ...
  private static boolean mapped(byte[] b) {
    for (int i = 0; i < 10; i++) if (b[i] != 0) return false;
    return (b[10] == 0 && b[11] == 0) || (b[10] == (byte) 0xff && b[11] == (byte) 0xff);
  }

  private static boolean starts(byte[] b, byte[] p) {
    for (int i = 0; i < p.length; i++) if (b[i] != p[i]) return false;
    return true;
  }

  private static boolean prefix(byte[] b, long p, int bits) {
    int whole = bits / 8, rem = bits % 8, count = (bits + 7) / 8;
    for (int i = 0; i < whole; i++)
      if ((b[i] & 255) != ((p >> ((count - i - 1) * 8)) & 255)) return false;
    return rem == 0
        || ((b[whole] & 255) & (0xff << (8 - rem))) == ((p & 255) & (0xff << (8 - rem)));
  }
}
