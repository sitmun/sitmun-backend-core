...
public interface CartographyStyleRepository extends JpaRepository<CartographyStyle, Integer> {

  @Query(
      "select count(cs) from CartographyStyle cs where cs.defaultStyle = true and cs.cartography = :cartography")
  Integer countDefaultStyles(@Param("cartography") Cartography cartography);

  @Query(
      "select count(cs) from CartographyStyle cs where cs.defaultStyle = true and cs.cartography = :cartography and cs <> :style")
