...
@Component
public class ApplicationBackgroundPermissionResolver
  implements PermissionResolver<ApplicationBackground> {
  ...
  public boolean resolvePermission(User authUser, ApplicationBackground entity, String permission) {
    ...
    if (permission.equalsIgnoreCase(SecurityConstants.CREATE_PERMISSION)
      ...
      return (permissions.stream().map(UserConfiguration::getRole)
        ...
        && (permissions.stream().map(UserConfiguration::getRole)
        .anyMatch(entity.getBackground().getCartographyGroup().getRoles()::contains))
        ;
    }
