...
class PointToStringConverterTest {
  ...
  void processPointsWithoutDecimals() {
    Point point = Point.builder().x(430492.0).y(4611482.0).build();
  ...
  void processPointsWithNull() {
    Point point = Point.builder().x(null).y(null).build();
    String string = converter.convertToDatabaseColumn(point);
    assertNull(string);
  }
  ...
  void processPointsWithDecimals() {
    Point point = Point.builder().x(430492.3).y(4611482.33).build();
    String string = converter.convertToDatabaseColumn(point);
    assertNotNull(string);
