...
class ProxyConfigurationServiceTest {
  ...
  void getConfigurationReturnsOgcWmsPayloadForWmsService() {
    ...
    assertInstanceOf(OgcWmsPayloadDto.class, result.getPayload());

    OgcWmsPayloadDto payload = (OgcWmsPayloadDto) result.getPayload();
  ...
  void getConfigurationReturnsNullForNonExistentService() {
    ...
    // When & Then
    assertThrows(
        org.sitmun.authorization.exception.BadRequestException.class,
        () -> service.getConfiguration(request, 0L));
  }
  ...
  void validateUserAccessReturnsTrue() {
    // Given
    ConfigProxyRequest request =
        ConfigProxyRequest.builder().appId(1).terId(1).type("WMS").typeId(1).method("GET").build();
