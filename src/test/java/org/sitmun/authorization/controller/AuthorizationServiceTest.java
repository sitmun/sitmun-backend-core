...
class AuthorizationServiceTest {
  ...
  void application() {
    Optional<Application> app =
        authorizationService.findApplicationByIdAndUserAndTerritory("public", 1, 1);
    assertTrue(app.isPresent());
    app.ifPresent(
        application -> assertThat(application.getName()).isEqualTo("SITMUN - Provincial"));
  }
  ...
  void roles() {
    List<Role> roles =
        authorizationService.findRolesByApplicationAndUserAndTerritory("public", 1, 1);
