...
class RoleRepositoryDataRestTest {
  ...
  void getApplicationsOfARole() throws Exception {
    mvc.perform(get(URIConstants.ROLE_APPLICATIONS_URI, 1).with(user(Fixtures.admin())))
  ...
  void getTasksOfARole() throws Exception {
    mvc.perform(get(URIConstants.ROLE_TASKS_URI, 1).with(user(Fixtures.admin())))
        .andExpect(status().isOk())
  ...
  void getPermissionsOfARole() throws Exception {
    mvc.perform(get(URIConstants.ROLE_PERMISSIONS_URI, 1).with(user(Fixtures.admin())))
        .andExpect(status().isOk())
