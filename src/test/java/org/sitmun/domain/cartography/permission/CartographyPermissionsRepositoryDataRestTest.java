...
class CartographyPermissionsRepositoryDataRestTest {
  ...
  void filterType() throws Exception {
    mvc.perform(
            get(
                    URIConstants.CARTOGRAPHY_PERMISSIONS_URI_FILTER,
        ...
        .andExpect(
            jsonPath(
                "$._embedded.cartography-groups[?(@.type == '"
                    + CartographyPermission.TYPE_SITUATION_MAP
                    + "')]",
                hasSize(1)));
    mvc.perform(
        ...
        .andExpect(
            jsonPath(
                ...
                hasSize(1)));
    mvc.perform(
            get(URIConstants.CARTOGRAPHY_PERMISSIONS_URI_FILTER, "C")
                .with(SecurityMockMvcRequestPostProcessors.user(Fixtures.admin())))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$._embedded.*.*", hasSize(1)))
  ...
  void filterOrType() throws Exception {
    mvc.perform(
            get(
                    URIConstants.CARTOGRAPHY_PERMISSIONS_URI_OR_FILTER,
                    CartographyPermission.TYPE_SITUATION_MAP,
                    "C")
                .with(SecurityMockMvcRequestPostProcessors.user(Fixtures.admin())))
        .andExpect(status().isOk())
  ...
  void rolesOfAPermissions() throws Exception {
    mvc.perform(
            get(URIConstants.CARTOGRAPHY_PERMISSION_ROLES_URI, 1)
                .with(SecurityMockMvcRequestPostProcessors.user(Fixtures.admin())))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$._embedded.*.*", hasSize(1)));
  }
  ...
  void createPermission() throws Exception {
    String content = '{' + "\"name\":\"test\"," + "\"type\":\"C\"" + '}';
    String location =
        mvc.perform(
                post(URIConstants.CARTOGRAPHY_PERMISSIONS_URI)
                    .content(content)
                    .with(SecurityMockMvcRequestPostProcessors.user(Fixtures.admin())))
    ...
    String changedContent = '{' + "\"name\":\"test2\"," + "\"type\":\"M\"" + '}';
  ...
  void cantChangeTypeWhenInUseAsSituationMap() throws Exception {
    String changedContent = '{' + "\"name\":\"test\"," + "\"type\":\"C\"" + '}';
    mvc.perform(
            put(URIConstants.CARTOGRAPHY_PERMISSION_URI, 3)
                .content(changedContent)
                .with(SecurityMockMvcRequestPostProcessors.user(Fixtures.admin())))
  ...
  void cantChangeTypeWhenInUseAsBackgroundMap() throws Exception {
    String changedContent = '{' + "\"name\":\"test\"," + "\"type\":\"C\"" + '}';
    mvc.perform(
            put(URIConstants.CARTOGRAPHY_PERMISSION_URI, 2)
                .content(changedContent)
                .with(SecurityMockMvcRequestPostProcessors.user(Fixtures.admin())))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.errors[0].invalidValue").value("C"));
